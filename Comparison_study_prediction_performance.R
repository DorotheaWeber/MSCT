...
# 5.1 Comparison Study - Prediction performance
...
load( "CV_splits.RData" ) # CV splits for 10-fold cross validation (balanced)
...
load( paste("IC50_data_", drugname, ".RData", sep="") ) # name "data"
  ...
  if(length(which(data[,1]==1))>3 && length(which(data[,1]==0))>3 && length(data[,1])==40){
    ...
    # leave 1/10 of data out for testing --> Cross Validation
    
    for(j in 1:10){
      
      # spilt in training and test set
      
      train = data[-CV_Splits[[drugname]][[j]],]
      levels(train[,1])=c("j", "n")
      train[,1] = relevel(train[,1], "j")
      test = data[CV_Splits[[drugname]][[j]],]
      levels(test[,1])=c("j", "n")
      test[,1] = relevel(test[,1], "j")
      
      # set name for output
      name = paste("result_", drugname, "_", j, sep="")
      ...
      filepath=paste("Results_200BS_",drugname, sep="")
      dir.create(filepath)
      save(result, file=paste(filepath, "/CCLE_melanoma_result_", drugname, "_CV", j, ".RData", sep=""))
